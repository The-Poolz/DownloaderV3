...
namespace DownloaderV2.Tests
{
    public class DownloadHandlerTests
    {
        ...
        public DownloadHandlerTests()
        {
            ...
            BaseDownloaderContext context = DbMock.CreateMockContextAsync().Result;

            Mock<SqlQueryHelper> mockSqlQueryHelper = new(context);
            _downloadHandler = new DownloadHandler(context);

            mockSqlQueryHelper.Setup(s => s.UpdateDownloaderSettings(It.IsAny<DownloaderSettings>(), It.IsAny<long>(), It.IsAny<long>()));
        }

        [Fact]
        public void AddResult_ShouldStoreCorrectDataInResultBuilder()
        {
            var mockSettings = DownloaderSettingsResult.SwapBNBPartyV1;
